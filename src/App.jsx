...
// Biểu đồ vòng tròn (Đã tối ưu kích thước cho Mobile)
const CircularProgress = ({ value, mainText, subText, max, colorClass, trailColor = "text-slate-100" }) => {
  const radius = 36; // Giảm một chút để vừa vặn trên điện thoại
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const savedData = localStorage.getItem('gpa-master-v11');
      if (savedData) {
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem('gpa-master-v11', JSON.stringify({ scores, subjectsList }));
    }
  ...
  return (
    // ĐÃ FIX: Cho phép cuộn trang tự nhiên trên Mobile (min-h-screen thay vì h-screen)
    <div className="min-h-screen bg-[#f0f4f8] p-0 md:p-6 font-sans text-slate-800 flex justify-center relative">
      {/* ĐÃ FIX: Bỏ overflow-hidden trên Mobile để vuốt mượt mà */}
      <div className="w-full max-w-[1280px] bg-white md:rounded-[24px] shadow-none md:shadow-2xl flex flex-col md:flex-row min-h-screen md:min-h-[90vh] md:overflow-hidden border-0 md:border border-slate-200">
        ...
        <div className="w-full md:w-[280px] bg-[#f8fafc] border-b md:border-b-0 md:border-r border-slate-200 p-4 md:p-6 flex flex-col shrink-0 relative z-10">
          ...
          {/* Dashboard Panel Nhỏ - ĐÃ FIX 2 VÒNG TRÒN NẰM NGANG */}
          <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200 shadow-sm flex flex-col items-center mb-5 md:mb-6">
            ...
            {/* Đây là đoạn fix cho 2 biểu đồ xếp ngang */}
          ...
          <div className="mt-2 md:mt-auto bg-gradient-to-br from-blue-500 to-cyan-400 rounded-2xl p-4 md:p-5 shadow-md text-white mb-2 md:mb-0">
            ...
            <p className="text-[13px] md:text-[14px] font-medium leading-relaxed min-h-[60px]">
              {displayedQuote}
        ...
        {/* Nội dung chính */}
        {/* ĐÃ FIX: overflow-visible trên Mobile để cuộn mượt */}
                ...
                return (
                  <div key={subject.id} className="bg-white rounded-2xl p-4 md:p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow relative group">
                    <button 
                      ...
                      className="absolute top-3 right-3 md:top-4 md:right-4 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
                      title="Xóa môn này"
                    >
                    ...
                    {subject.hasFinal && (
                      <div className="mt-4 md:mt-5 pt-3 md:pt-4 border-t border-slate-100 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-4">
                        ...
                        <div className="flex items-center justify-between md:justify-start gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                          <span className="text-[11px] md:text-[12px] text-slate-500 font-bold ml-1">Mục tiêu:</span>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                            {reqA > 0 && reqA <= 10 && numTarget > 0 && (
                              <button
                                ...
                                className="text-[12px] bg-white border border-blue-200 hover:bg-blue-50 text-blue-600 px-2 md:px-3 py-1.5 rounded-lg transition-colors font-bold shadow-sm active:scale-95 cursor-pointer flex items-center gap-1"
                              >
                                ⚡ Tự điền
                              </button>
                            )}
                            <span className={`font-bold px-2 md:px-3 py-1.5 rounded-lg text-[12px] md:text-[13px] border ${numTarget <= 0 ? 'bg-slate-50 text-slate-500 border-slate-200' : (reqA <= 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : (reqA > 10 ? 'bg-red-50 text-red-600 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200 shadow-sm'))}`}>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div>
                      ...
                      <input type="number" disabled={!newSub.hasFinal} value={newSub.hasFinal ? newSub.finW : 0} onChange={e => setNewSub({...newSub, finW: e.target.value})} 
                        className={`w-full border border-slate-300 rounded-lg px-1 md:px-2 py-2 text-center text-[14px] ${!newSub.hasFinal ? 'bg-slate-200 text-slate-400' : ''}`} />
                    </div>
                  </div>
                </div>
              </div>
