...
const TypingText = ({ text, colorClass = "text-white" }) => {
  ...
  return (
    <p className={`${colorClass} font-bold leading-relaxed text-[13px] mt-2`}>
      {displayedText}
    ...
    <div className="flex flex-col items-center">
      <div className="relative flex items-center justify-center w-24 h-24 md:w-32 md:h-32 mb-3">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-[20px] md:text-[28px] font-black text-slate-800">{mainText}</span>
        </div>
      </div>
      <span className="text-[12px] font-black text-slate-500 uppercase tracking-widest">{subText}</span>
    </div>
  );
};
...
export default function App() {
  ...
  const updateScore = (id, field, val) => {
    ...
    setScores(prev => {
      ...
      if (field === 'targetType') {
        return { ...prev, [id]: { ...currentData, targetType: cleanVal, target: cleanVal === 'letter' ? 'A' : '8.5' } };
      }
      return { ...prev, [id]: { ...currentData, [field]: cleanVal } };
    });
  };

  const toggleSelectMode = () => { setIsSelectionMode(!isSelectionMode); setSelectedIds([]); };
  const toggleSelect = (id) => { setSelectedIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]); };
  const selectAll = () => { if (selectedIds.length === subjectsList.length) setSelectedIds([]); else setSelectedIds(subjectsList.map(s => s.id)); };
  ...
  const handleBulkAdd = (items) => {
    const newItems = items.map(it => ({ ...it, id: "BULK_" + Math.random().toString(36).substr(2, 9) }));
  ...
  const handleAddSubject = (keepOpen = false) => {
    ...
    const att = Number(newSub.attW) || 0; const mid = Number(newSub.midW) || 0; const fin = Number(newSub.finW) || 0;
    ...
    setScores(prev => ({ ...prev, [id]: { attendance: '', midterm: '', final: '', targetType: newSub.targetType, target: newSub.target } }));

    if (keepOpen) { setNewSub(initialNewSub); } else { setShowAddModal(false); setNewSub(initialNewSub); }
  };
